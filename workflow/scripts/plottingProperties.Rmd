...
## Summary for `r snakemake@wildcards$trait` traits
<br>
#This report shows summary statistics for predictions. <br> It seeks to give an overview of the properties of the candidate enhancer regions, enhancer regions in the prediction methods used. This will give us  a better understanding of why certain methods might perform better under different validation strategies. Some of these statistics include: <br>
...
<br>
**Fig1. Number / Width / log2(DNase) / log2(H3K27ac) of Candidate Enhancer Regions and Enhancer Regions.** Summary of properties of candidate enhancer regions and enhancer regions across all biosamples. **a**, Width of candidate enhancer regions and enhancer regions (bp) across all biosamples. (top left) **b**, Number of candidate enhancer regions and enhancer regions across all biosamples. (top right) **c**, log2(DNase) counts of candidate enhancer regions and enhancer regions across all biosamples. (bottom left) **d**, log2(H3K27ac) counts of candidate enhancer regions and enhancer regions across all biosamples. (bottom right) 
...
cdfEnhAcetylation = ggplot(subset_predictionFile, aes(H3K27ac, col=Method)) +
	...
	theme(text = element_text(size = rel(4)), legend.text=element_text(size=rel(2.5)))
```
## Cumulative Density Plots comparing Width, Accessibility and Acetylation for Candidate regions (top) and enhancer regions (bottom) across all predictors
...
#plot_grid(cdfWidth, cdfAccessible, cdfAcetylation, cdfEnhWidth, cdfEnhAccessible, cdfEnhAcetylation, labels="AUTO", ncol=3)ggregateDF
