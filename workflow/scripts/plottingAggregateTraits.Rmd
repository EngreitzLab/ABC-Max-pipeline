...
#write.table(numCellTypesDF, file="numCellTypesDF.tsv")
...
```{r numEnrichedCellTypes_noPromoters, echo=FALSE, error=FALSE, message=FALSE, warnings=FALSE}
#cellEnrichmentSignificant <- cellEnrichment %>% filter(p < 0.05)
#numCellTypes <- cellEnrichmentSignificant %>% group_by(Disease) %>% mutate(numCellTypes=length(unique(cellEnrichmentSignificant$CellType)))
#numCellTypesPlot_noPromoters <- ggplot(numCellTypes, aes(x=Disease, y=numCellTypes, fill = Disease)) + ylab(paste0("Number of Enriched CellTypes")) + ggtitle(paste0("Enhancers (excluding promoter regions)"))
```
