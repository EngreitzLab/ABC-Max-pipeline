...
#knownGenes <- strsplit(snakemake@input$knownGenes, " ") %>% unlist()
...
allFlat = strsplit(snakemake@input$allFlat, " ") %>% unlist()
trait_params = read.delim(snakemake@params$traitTable, sep='\t', header=TRUE, stringsAsFactors = FALSE)
#
...
all.flat.full = read.csv(file=allFlat[1], sep='\t', header=TRUE, stringsAsFactors = FALSE)
...
if(length(allFlat)>1){
       for (i in 2:length(allFlat)){
               tryCatch(
               {
		       temp = read.delim(file=allFlat[i], sep='\t', header=TRUE, stringsAsFactors = FALSE)
...
all.v <- readAllVariants(trait_params, snakemake@params$traitDir)

# not necessary right now
final.flat.full <- NULL
final.flat.v <- NULL
...
write.tab(enrichVsPp, file=paste0(snakemake@params$indivOutDir, "/EnrichmentVsPosteriorProb.tsv"))
...
write.tab(enrichVsPp.noncoding, file=paste0(snakemake@params$indivOutDir, "/EnrichmentVsPosteriorProbNonCoding.tsv"))
...
p <- p #+ scale_color_manual(values = c(trait = 'blue',
	#			     '59 other traits' = 'azure3', 
	#			    'Average of all traits' = 'black',
	#			   '12 blood traits' = 'red') + labs(color="")) 
```
...
write.table(subsetGeneList, file="subsetGeneList.tsv")
if (length(names) > 1){
...
for (gl in colnames(subsetGeneList))
	gp.plot <- doOneKnownGeneList(gl, gp.all, predictors)
...
aggCurve_title <- aggCurve + ggtitle('C') 
...
<br>
**Fig1. `r snakemake@wildcards$pred` maps connecting fine-mapped variants to enhancer, genes and celltypes. a**, Enrichment of fine-mapped variants (PIP>10%) in `r snakemake@wildcards$pred` enhancers. **b**, Fraction of fine-mapped variants (PIP>10%) overlapping `r snakemake@wildcards$pred` enhancers. **c**,  Precision–recall plot of connections between noncoding credible sets and known genes. 
<br>
...
table.current = read.table(enrichmentTables[index], header=TRUE) %>% dplyr::select(CellType, Disease, enrichment, enrichment.NoPromoters, n, total, n.NoPromoters, total.NoPromoters )
cat("  \n")
...
aggCurve
cap <- paste0("Fig5. Precision–recall plot of connections between noncoding credible sets and known genes, where recall is the fraction of credible sets for which the known
...
their ability to identify disease-associated genes. . For this analysis, we used the predictions from each method to overlap fine-mapped variants
