...
## Set theme of plots
mytheme <- theme_classic() + theme(axis.text.y = element_text(angle = 45, vjust = 1, hjust = 1))
...
formatEnrichmentPointPlot <- function(p) p + xlim(0, 22) + mytheme + geom_vline(xintercept=1, linetype="dashed", color="gray") + scale_color_hue(l=50, c=90) + theme(plot.title = element_text(size=12), axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank())


ibdEnhancerList$title1 <- "Enhancers (excluding promoter regions)"
ibdEnhancerList$CellType <- ordered(ibdEnhancerList$CellType, levels=ibdEnhancerList[order(ibdEnhancerList$enrichment.NoPromoters),"CellType"])
top10 <- head(ibdEnhancerList[order(-ibdEnhancerList$enrichment.NoPromoters),"CellType"], 10)
p1 <- ggplot(ibdEnhancerList, aes(x=enrichment.NoPromoters, y=CellType)) + geom_point(size=1, alpha=0.3) + facet_wrap(~title1) + xlab(paste0("Enrichment\n( ", trait, " variants / all variants)")) + geom_point(data=subset(ibdEnhancerList, CellType %in% top10), aes(colour=CellType), size=4) + scale_color_discrete(name="CellType") + theme(legend.position="none") 
p1 <- p1 %>% formatEnrichmentPointPlot()
p1_label <- p1 

p1_plotly <- ggplot(ibdEnhancerList, aes(x=enrichment.NoPromoters, y=CellType, fill=CellType)) + facet_wrap(~title1) + ylab("CellTypes") + xlab(paste0("Enrichment\n( ", trait, " variants / all variants)")) + theme(legend.position="none") + geom_point()
p1_plotly <- p1_plotly %>% formatEnrichmentPointPlot() + theme(legend.position="none")

```

```{r plottingEnrichmentCode_all, echo=FALSE, error=FALSE, message=FALSE, warnings=FALSE}
#ibdEnhancerList$CellType <- ordered(ibdEnhancerList$CellType, levels=rev(ibdEnhancerList[order(ibdEnhancerList$enrichment),"CellType"]))
ibdEnhancerList$title2 <- "Enhancers (including promoter regions)"
p2 <- ggplot(ibdEnhancerList, aes(x=enrichment, y=CellType, fill=CellType)) + xlab(paste0("Enrichment\n( ", trait, " variants / all variants)")) + ylab("CellTypes") + facet_wrap(~title2) + theme(legend.position="none") + geom_point() #+ geom_label_repel(data = head(ibdEnhancerList[order(-ibdEnhancerList$enrichment),], 10), aes(label = CellType), color = "black") + theme(legend.position="none")
p2 <- p2 %>% formatEnrichmentPointPlot() + theme(legend.position="none")
p2_label <- p2 
```

```{r plottingFractionOverlap, echo=FALSE, error=FALSE, message=FALSE, warnings=FALSE}
formatFractionBarPlot <- function(p) p + mytheme + scale_color_hue(l=50, c=90) + theme(plot.title = element_text(size=12), axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank())

#ibdFractionOverlap <- enrichPlot
#ibdFractionOverlap$title1 <- "Enhancers (excluding promoter regions)"
#ibdFractionOverlap$title2 <- "Enhancers (including promoter regions)"
ibdFractionOverlap <- ibdEnhancerList
top10 <- head(ibdFractionOverlap[order(-ibdFractionOverlap$n.noPromoters.FractionOverlap),"CellType"], 10)

p3 <- ggplot(ibdFractionOverlap, aes(x=n.noPromoters.FractionOverlap, y=CellType)) + geom_point(alpha = 0.5) + xlab(paste0("Fraction of variants overlapping predicted enhancers \n( ", trait, " variants / all variants)")) + facet_wrap(~title1) + geom_point(data=subset(ibdFractionOverlap, CellType %in% top10), aes(colour=CellType), size=4) + scale_color_discrete(name="CellType") + theme(legend.position="none") + scale_x_continuous(limits = c(0, max(ibdFractionOverlap$n.noPromoters.FractionOverlap)))
p3 <- p3 %>% formatFractionBarPlot()

ibdFractionOverlap$CellType <- ordered(ibdFractionOverlap$CellType, levels=ibdFractionOverlap[order(ibdFractionOverlap$n.noPromoters.FractionOverlap),"CellType"])
p3_plotly <- ggplot(ibdFractionOverlap, aes(x=n.noPromoters.FractionOverlap, y=CellType, fill=CellType)) + xlab(paste0("Fraction of variants overlapping predicted enhancers \n( ", trait, " variants / all variants)")) + facet_wrap(~title1) + theme(legend.position="none") + geom_point()
p3_plotly <- p3_plotly %>% formatFractionBarPlot() + theme(legend.position="none") #+ scale_x_continuous(limits = c(0, max(ibdFractionOverlap$n.noPromoters.FractionOverlap))) + theme(legend.position="none")
```


```{r plottingFractionOverlap_all, echo=FALSE, error=FALSE, message=FALSE, warnings=FALSE, fig.width=6, fig.height=4}
ibdFractionOverlap$CellType <- ordered(ibdFractionOverlap$CellType, levels=ibdFractionOverlap[order(ibdFractionOverlap$n.FractionOverlap),"CellType"])
p4 <- ggplot(ibdFractionOverlap, aes(x=n.FractionOverlap, y=CellType, fill = CellType)) + xlab(paste0("Fraction of variants overlapping predicted enhancers \n( ", trait, " variants / all variants)")) + facet_wrap(~title2) + theme(legend.position="none") #+ geom_label_repel(data = head(ibdFractionOverlap[order(-ibdFractionOverlap$n.FractionOverlap),], 10), aes(label = CellType), color = "black") + theme(legend.position="none") 
p4 <- p4 %>% formatFractionBarPlot() + scale_x_continuous(limits = c(0, max(ibdFractionOverlap$n.FractionOverlap))) + theme(legend.position="none") + geom_point()
p4_label <- p4 
```
...
getPRPlot <- function(pr, baseline=NULL, xlab="Recall") {
      #PredictionType <- strsplit(pr$Method, split = ".")
      ...
      numMethods <- length(unique(pr$Predictor)) - 2
      Methods <- unique(pr[(grep("^Closest", pr$Predictor, invert=TRUE)), "Predictor"])  
      ...
      for (i in 1:numMethods){
        Method <- Methods[i]
        index = grep(as.character(paste0(Method, "")), pr$Predictor)
        pr$Group[index] <- i+2 	
      }
      ...
      p <- ggplot(pr.points, aes(x=Recall, y=Precision, shape=PredictionType, color=Predictor)) #+ geom_line(aes(group = Group))
      if (!is.null(baseline)) p <- p + geom_hline(yintercept=baseline, color='gray', linetype='dashed')
      p <- p + geom_point(size=3)
      p <- p + geom_line(aes(group = Group))#data=pr.lines)
...
```{r summary,  echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.width=18, fig.height=5}
...
ptly1 <- ggplotly(p1_plotly)
ptly2 <- ggplotly(p2_label)
subplot(ptly1, ptly2, nrows=1, titleX = TRUE, titleY=TRUE)
...
# What fraction of `r snakemake@wildcards$trait` variants overlap predicted enhancers?
...
ptly3 <- ggplotly(p3_plotly)
ptly4 <- ggplotly(p4_label)
subplot(ptly3, ptly4, nrows=1, titleX = TRUE)
