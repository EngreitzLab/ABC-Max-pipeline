...
ibdEnhancerList <- cellEnrichment
...
p2 <- ggplot(ibdEnhancerList, aes(x=enrichment, y=CellType, fill=CellType)) + xlab(paste0("Enrichment\n( ", trait, " variants / all variants)")) + ylab("CellTypes") + facet_wrap(~title2) + theme(legend.position="none") + geom_point() 
...
p3_plotly <- p3_plotly %>% formatFractionBarPlot() + theme(legend.position="none") 
```
...
for (gl in colnames(knownGenes))
	gp.plot <- doOneKnownGeneList(gl, gp, predictors, knownGenes)
...
pdf(file="pr.curve.pdf", width=5, height=4)
...
p1_title <- p1 
p3_title <- p3 
singlePR_title <- singlePR 
aggCurve_title <- aggCurve 
#fig2_title <- fig2 + ggtitle('E') 
#fig2a_title <- fig2a + ggtitle('F') 

ggarrange(p1_title, p3_title, labels=c("A", "B"), nrow=1)
```
```{r summary1, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, fig.width=10, fig.height=5}
ggarrange(singlePR_title, aggCurve_title, labels=c("C", "D"), nrow=1)
```
<br>
**Fig1. `r snakemake@wildcards$pred` maps connecting fine-mapped variants to enhancer, genes and celltypes. a**, Enrichment of fine-mapped `r snakemake@wildcards$trait` variants (PIP>10%) in `r snakemake@wildcards$pred` enhancers. **b**, Fraction of fine-mapped `r snakemake@wildcards$trait` variants (PIP>10%) overlapping `r snakemake@wildcards$pred` enhancers. **c**, Precision–recall plot of connections between noncoding `r snakemake@wildcards$trait`  credible sets and known `r snakemake@wildcards$trait`-associated genes. **d**,  Precision–recall plot of connections between noncoding `r snakemake@wildcards$trait`  credible sets and known `r snakemake@wildcards$trait`-associated genes across different prediction methods. 
<br>
