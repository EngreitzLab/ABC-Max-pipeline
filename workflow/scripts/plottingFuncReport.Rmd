...
p1 <- ggplot(ibdEnhancerList, aes(x=enrichment.NoPromoters, y=CellType, color=CellType)) + facet_wrap(~title1) + xlab(paste0("Enrichment\n( ", trait, " variants / all variants)")) + geom_label_repel(data = head(ibdEnhancerList[order(-ibdEnhancerList$enrichment.NoPromoters),], 10), aes(label = CellType), color = "black") + theme(legend.position="none")	
...
p2 <- ggplot(ibdEnhancerList, aes(x=enrichment, y=CellType, color=CellType)) + xlab(paste0("Enrichment\n( ", trait, " variants / all variants)")) + facet_wrap(~title2) + theme(legend.position="none") #+ geom_label_repel(data = head(ibdEnhancerList[order(-ibdEnhancerList$enrichment),], 10), aes(label = CellType), color = "black") + theme(legend.position="none")
p2 <- p2 %>% formatEnrichmentBarPlot()
p2_label <- p2 
```

```{r plottingFractionOverlap, echo=FALSE, error=FALSE, message=FALSE, warnings=FALSE}
formatFractionBarPlot <- function(p) p + geom_jitter(position=position_jitter(0.2), size=3) +  mytheme + scale_color_hue(l=50, c=90) + theme(axis.title.y=element_blank(), axis.text.y=element_blank(), axis.ticks.y=element_blank()) + theme(legend.position="none")

ibdFractionOverlap <- ibdEnhancerList
#ibdFractionOverlap$CellType <- ordered(ibdFractionOverlap$CellType, levels=rev(ibdFractionOverlap[order(ibdFractionOverlap$n.noPromoters.FractionOverlap),"CellType"]))
p3 <- ggplot(ibdFractionOverlap, aes(x=n.noPromoters.FractionOverlap, y=CellType, color = CellType)) + xlab(paste0("Fraction Overlap\n( ", trait, " variants / all variants)")) + facet_wrap(~title1) + geom_label_repel(data = head(ibdFractionOverlap[order(-ibdFractionOverlap$n.noPromoters.FractionOverlap),], 10), aes(label = CellType), color = "black") + theme(legend.position="none")
p3 <- p3 %>% formatFractionBarPlot() + scale_x_continuous(limits = c(0, max(ibdFractionOverlap$n.noPromoters.FractionOverlap))) 
p3_label <- p3 #+ geom_label_repel(data=cellTypeLabels, aes(x=CellType, y=n.noPromoters.FractionOverlap, label=CellType))
```
