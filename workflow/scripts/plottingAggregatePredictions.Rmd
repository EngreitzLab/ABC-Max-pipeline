...
numCellTypesFiles = strsplit(snakemake@input$enrichedCellTypes, " ") %>% unlist()
enrichVsPp.noncoding.file = strsplit(snakemake@input$enrichVsPp_noncoding, " ") %>% unlist()
allgenePredTable = strsplit(snakemake@input$allgenePredTableFiles, " ") %>% unlist()
#PRtableFiles = strsplit(snakemake@input$PRtableFiles, " ") %>% unlist()
knownGenes = read.delim(snakemake@input$geneLists, sep="\t", header=TRUE, check.names=F, stringsAsFactors=F, row.names=NULL)
...
# Set theme of plots
mytheme <- theme_classic() + theme(axis.text.x = element_text(angle = 90, vjust = 1, hjust = 1), axis.text = element_text(size = 13), axis.title = element_text(size = 15))
aggregateDF <- cellEnrichment
ibdEnhancerList <- cellEnrichment[, c("CellType", "Disease", "enrichment", "enrichment.NoPromoters", "n.noPromoters.FractionOverlap", "n.FractionOverlap", "Method")]
#is_method <- ibdEnhancerList$Method == pred
#bdEnhancerList <- ibdEnhancerList[is_method,]
diseases <- unique(ibdEnhancerList$Disease)
#formatEnrichmentBarPlot <- function(p) p + geom_point() + mytheme() 
...
p1 <- ggplot(ibdEnhancerList, aes(x=Disease, y=enrichment.NoPromoters, fill=Disease)) + ylab(paste0("Enrichment\n( GWAS variants / all variants)")) + labs(title=paste0("Enhancers (excluding promoter regions) across traits")) + xlab("Trait") + ylim(0, max(ibdEnhancerList$enrichment.NoPromoters)) + facet_wrap(~ Method, ncol=1)
p1 <- p1 %>% formatEnrichmentBarPlot()
```

```{r plottingEnrichmentCode_all, echo=FALSE, error=FALSE, message=FALSE, warnings=FALSE}
p2 <- ggplot(ibdEnhancerList, aes(x=Disease, y=enrichment, fill=Disease)) + ylab(paste0("Enrichment\n( GWAS variants / all variants)")) + labs(title=paste0("Enhancers (including promoter regions)")) + xlab("Trait") + ylim(0, max(ibdEnhancerList$enrichment)) + facet_wrap(~ Method, ncol=1)
...
p4 <- ggplot(ibdEnhancerList, aes(x=Disease, y=n.FractionOverlap, fill = Disease)) + ylab(paste0("Fraction Overlap\n( GWAS variants / all variants)")) + ggtitle(paste0("Enhancers (including promoter regions)")) + facet_wrap(~ Method, ncol=1) 
p4 <- p4 %>% formatEnrichmentBarPlot()
#ggplotly(p4)
```

```{r numEnrichedCellTypes, echo=FALSE, error=FALSE, message=FALSE, warnings=FALSE}
numCellTypesDF <- read.delim(numCellTypesFiles[1], sep="\t", header=TRUE, check.names=F,stringsAsFactors=F, row.names=NULL)
numCellTypesDF$Method <- all_predictors[1]

numCellTypesDF <- transform(numCellTypesDF, n=n/max(numCellTypesDF$n))

if (length(numCellTypesFiles) > 1){
	for (i in 2:length(numCellTypesFiles)){
		tryCatch(
		{
			temp = read.delim(file=numCellTypesFiles[i], sep='\t', header=TRUE, stringsAsFactors = FALSE)
			temp$Method <- all_predictors[i]
			temp <- transform(temp, n/max(temp$n))
			numCellTypesDF = rbind(numCellTypesDF, temp)
		},
		error=function(cond) {
			message(paste("File does not exist::", i))
		},
...
numCellTypesPlot_noPromoters <- ggplot(numCellTypesDF, aes(x=Disease, y=n, fill=Disease)) + ylab(paste0("Number of Enriched CellTypes / Total Number of CellTypes")) + ggtitle(paste0("Enhancers (excluding promoter regions)")) + mytheme + geom_point() + scale_color_manual(values=diseases) + scale_color_hue(l=50, c=90) + theme(legend.position='none') + scale_y_continuous(breaks=seq(0,max(numCellTypesDF$n),0.1)) + facet_wrap(~Method, ncol=1)
...
# Concatentate enrichVsPp.noncoding 
enrichVsPp.noncoding <- read.delim(enrichVsPp.noncoding.file[1], sep="\t", header=TRUE, check.names=F,stringsAsFactors=F, row.names=NULL)
enrichVsPp.noncoding$Method <- all_predictors[1]

if (length(enrichVsPp.noncoding.file) > 1){
	for (i in 2:length(enrichVsPp.noncoding.file)){
		tryCatch(
		{
			temp = read.delim(file=enrichVsPp.noncoding.file[i], sep='\t', header=TRUE, stringsAsFactors = FALSE)
			temp$Method <- all_predictors[i]
			enrichVsPp.noncoding = rbind(enrichVsPp.noncoding, temp)
		},
		error=function(cond) {
			message(paste("File does not exist::", i))
		},
...
trait="IBD"
p <- ggplot(subset(enrichVsPp.noncoding, nTotal >= 5 & PosteriorProb <= 0.95), aes(x=log10(PosteriorProb), y=fraction*100, group=Disease)) + geom_line(color='gray') + xlab("PIP (log10)") + ylab("% noncoding variants") + ylim(0,100) + mytheme + facet_wrap(~ Method, ncol=3)
if (dim(subset(enrichVsPp.noncoding,Disease%in%bloodRelatedTraits)[1]) != 0) {
...
gp.all <- read.delim(allgenePredTable[1], sep="\t", header=TRUE, check.names=F, stringsAsFactors=F, row.names=NULL)
gp.all$Method <- all_predictors[1]
mergecols <- colnames(gp.all[, 1:10])

if (length(allgenePredTable) >1){
	for (i in 2:length(allgenePredTable)){
		tryCatch(
		{
			# PLOT Aggregate Precision-Recall Curves
			temp <- read.delim(allgenePredTable[i], sep="\t", header=T, check.names=F, stringsAsFactors=F)
			temp$Method <- all_predictors[1]
...
# get total number of knownGenes 
#knownGenes <- read.delim(knownGenesFiles[1], sep="\t", header=TRUE, check.names=F, stringsAsFactors=F, comment.char='#') 
#colnames(knownGenes) <- "GeneList"
#for (i in 2:length(knownGenesFiles)){
#	temp <- read.delim(knownGenesFiles[i], sep="\t", header=TRUE, check.names=F, stringsAsFactors=F, comment.char='#') 
#	colnames(temp) <- "GeneList"
#	knownGenes <- rbind(knownGenes, temp)	
#}
#knownGenes <- knownGenes %>% distinct()
...
for (gl in colnames(knownGenes))
	gp.plot <- doOneKnownGeneList(gl, gp.all, predictors, knownGenes, maxKnownGenes=length(knownGenes))
...
p3_title
#patchwork
numCellTypesPlot_noPromoters_title
patchwork1 <- aggCurve_title + cdfAgg
patchwork1
```
...
```{r enrichmentplot1, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.cap=cap, fig.width=14, fig.height=10}
...
for (i in 1:length(all_predictors)){
	cat("  \n###",  all_predictors[i], "  \n")
	table.current = subset(cellEnrichment, Method== paste0(all_predictors[i], ""))
	cat("  \n")
	...
	cat(knitr::knit_print(reactable(table.current, striped=TRUE, searchable=TRUE, filterable=TRUE,
					...
					columns = list(enrichment = colDef(name = "GWAS enrichment (including promoter regions", format=colFormat(digits=2)), 
					enrichment.NoPromoters = colDef(name = "GWAS enrichment (excluding promoter regions)", format=colFormat(digits=2)),
					n.FractionOverlap = colDef(name = "# variants in Enhancers"),
					n.noPromoters.FractionOverlap = colDef(name = "# variants in Enhancers (excluding promoter regions)")))))
	cat("  \n")
	cat("  \n")
}
```
...
We plot the fraction overlap of variants (PIP >= 10%) that overlap enhancers with and without promoter regions (promoter regions as defined by RefSeq) across all biosamples.
```{r enrichmentplot3, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.cap=cap, fig.width=14, fig.height=10}
ptly3 <- ggplotly(p3)
ptly4 <- ggplotly(p4)
...
```{r plotEnriched, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.cap=cap, fig.height=14, fig.width=10}
...
# How does the fraction of noncoding variants that overlap a given enhancer in any biosample change given the PIP threshold?
...
```{r enrichmentplot5, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, fig.cap=cap, fig.height=4, fig.width=10}
ggplotly(p)
cap <- paste0("Fig5. Fraction of noncoding variants above a given PIP threshold that overlap an enhancer in any biosample. Black line, weighted average across traits. Traces are shown for PIP thresholds above which there are at least five variants. Dashed line, fraction of all common noncoding variants that overlap enhancers.")
```
