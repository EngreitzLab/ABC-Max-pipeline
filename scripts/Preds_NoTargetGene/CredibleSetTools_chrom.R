...
loadVariantOverlap <- function(overlap.file, genes.uniq, genes, variant.names=NULL, colMap=NULL, overwriteTSS=FALSE) {
  ...
  #$TargetGeneIsCoding <- as.character(as.matrix(x$TargetGene)) %in% codingSymbols
...
getVariantByGenesTable <- function(overlap) {
  variant.by.genes <- overlap %>% group_by(QueryRegionName) %>% summarise( n.cells=n(), CellTypes=paste0(CellType,collapse=',')) %>% as.data.frame()
...
computeCellTypeEnrichment <- function(variants.by.cells, variant.list, cell.type.annot, trait, cell.group.by=NULL, score.col="PosteriorProb", min.score=0.1, bg.vars=NULL, bg.overlap=NULL, noPromoter=FALSE) {
  ...
  bg_V8 <- bg.overlap %>% distinct(V4,V8) 
  bg_V8_count <- bg_V8 %>% group_by(V8) %>% tally()
  ...
  write.tab(hi.count, file="TraitCount.tsv")
  write.tab(bg_V8_count, file="BackgroundCount.tsv")
  hi.count$n.ctrl <- bg_V8_count$n
