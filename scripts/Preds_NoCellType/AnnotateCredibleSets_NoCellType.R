...
suppressPackageStartupMessages(source(paste0(opt$codeDir, "CredibleSetTools_NoCellType.R")))
...
cell.type.list <- "CELLTYPE" #unique(data$V8)
...
#write.tab(all.flat, file="original.all.flat.tsv")
#write.tab(cell.type.list, file="cell.type.list.tsv")
#all.flat <- subset(all.flat, CellType %in% cell.type.list)  ## IMPORTANT CHANGE
#write.tab(all.flat, file="all.flat.tsv")
filter.flat <- subset(all.flat, CredibleSet %in% filter.cs$CredibleSet)
#write.tab(filter.flat, file="filter.flat.tsv")
...
variant.by.cells <- getVariantByCellsTable(filter.flat)
write.tab(variant.by.cells, file="NoCellTYPE.variant.by.cells.tsv")
...
#bgOverlapPerCelltype <- bgOverlap %>% group_by(V8) %>% summarise(prop.overlap=n()/nrow(bgOverlap))
...
if (!(is.null(opt$promoters))) {
  ...
  enrich.nopromoter <- computeCellTypeEnrichment(getVariantByCellsTable(subset(filter.flat, !Promoter), opt$variantScoreCol),
						 ...
						 trait,
                                                 score.col=opt$variantScoreCol,
                                                 min.score=opt$variantScoreThreshold, bg.overlap=bgOverlap, bg.vars=bgVars)
...
#                                              cell.type.annot, trait, cell.group.by=cat, 
#                                              score.col=opt$variantScoreCol,
#                                              min.score=opt$variantScoreThreshold,
#                                              bg.vars=bgOverlap) # NOT SUPPORTED YET, ldsc=ldsc[[trait]])
#
